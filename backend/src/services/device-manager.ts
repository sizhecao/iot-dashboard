...
export class DeviceManager {
  ...
  private validateDeviceConfig(
    ...
    if (config.updateInterval && config.updateInterval < 1000) {
      throw new InvalidDeviceConfigError(
        'Update interval must be at least 1000ms'
      );
    }
    ...
    if (type === 'temperature') {
      if (
        ...
        (config.minValue < -50 || config.minValue > 100)
      ) {
        throw new InvalidDeviceConfigError(
      ...
      if (
        ...
        (config.maxValue < -50 || config.maxValue > 100)
      ) {
        throw new InvalidDeviceConfigError(
          'Temperature must be between -50°C and 100°C'
        );
    ...
    if (type === 'humidity') {
      if (
        config.minValue !== undefined &&
        (config.minValue < 0 || config.minValue > 100)
      ) {
        throw new InvalidDeviceConfigError(
      ...
      if (
        config.maxValue !== undefined &&
        (config.maxValue < 0 || config.maxValue > 100)
      ) {
        throw new InvalidDeviceConfigError(
          'Temperature must be between 0% and 100%'
        );
    ...
    if (
      ...
      throw new InvalidDeviceConfigError(
        'Minimum must be less than maxinmum vvalue.'
      );
    }
  }
  ...
  public removeDevice(deviceId: string): void {
    ...
    wsService.broadcast('deviceRemoved', deviceId);
